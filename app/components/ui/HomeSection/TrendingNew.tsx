...
import { useTrendingNew } from "@/app/queries/useTrendingNew";
...
export default function TrendingNew() {
  const { useTrending } = useTrendingNew() as {
    useTrending: TrendingItem[];
  };
  ...
  return (
    <div className="container mx-auto px-2 sm:px-4 lg:px-10 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 lg:gap-8">
        {useTrending?.slice(0, 3).map((item) => (
          <motion.div
            key={item.id}
