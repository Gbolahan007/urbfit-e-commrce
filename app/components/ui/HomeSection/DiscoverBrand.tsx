...
import { useHomeBrands } from "@/app/queries/useBrands";
import Image from "next/image";
import { motion } from "framer-motion";

export default function DiscoverBrand() {
  const { homeBrands } = useHomeBrands();
    ...
    <div>
      ...
      <div className="mx-auto max-w-6xl px-4 pb-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {homeBrands?.slice(0, 6).map((brand) => (
            <motion.div
              key={`brand-${brand.id}`}
            ...
            >
              <Link
                href={`/brands/${brand.slug}`}
                className="group block overflow-hidden hover:shadow-2xl transition-all duration-300 rounded-lg"
              >
